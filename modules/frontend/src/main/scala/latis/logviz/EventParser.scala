...
object EventParser {
  def apply(): IO[EventParser] = {
    for {
      ...
      //actually I dont think I need to offer when I need it, I can just pass in the col counter and minus from total to know when to stop looking
      _ 						<- (0 to 99).toList.traverse(i => pq.offer(Column(i, false)))
      // _             <- pq.offer(Column(1, false))
      ...
      */
      ...
      override def parse(event: Event): IO[Unit] =
        event match {
          case Event.Start(time) =>
            for {
              cDepth	<- pq.take
              usedCol =  cDepth.copy(used = true) //can set to true later after if care about keeping track of the first event of that column
            ...
            if (status >= 400) {
              for {
                ...
                cDepth  <- map.get(id) match {
                            ...
                            case None => 
                              //we have an event whose request is outside the time range so start time is unknown
                              //and since status is 400, its completed
              ...
              // IO.unit //we have an ongoing partial event. 
              //maybe we dont need to store since we're not saving anything like status or anything
              for {
                map <- eventsRef.get
                _   <- map.get(id) match {
                        case Some(_) => IO.unit
      ...
      override def getEvents(): IO[List[(RequestEvent, Int)]] =
        for {
          ...
          test        = compEvents.map((event, col) => (event, col.number))
        } yield(test ++ events)
